...
namespace dev.mikeee324.OpenPutt
{
    ...
    public class GolfClubCollider : UdonSharpBehaviour
    {
        ...
        [Tooltip("Maps how square the club face is to the ball's travel direction (1 = dead square, 0 = 90deg+ off) onto a side spin amount (1 = full spin, 0 = none). Leave empty to use the built-in default curve.")]
        public AnimationCurve sideSpinMagnitudeCurve;

        [Range(0f, 5f), Tooltip("Overall multiplier for how much side spin (hook/slice) is generated. 1 = default, higher = more curve.")]
        public float sideSpinMultiplier = 1.0f;

        [Range(0f, 5f), Tooltip("Multiplier for backspin generated by loft. Backspin becomes (capped) lift on the ball, so higher = more float/carry and softer landings on lofted shots. 0 = no backspin (pure ballistic loft).")]
        ...
        public void HandleBallHit(Vector3 overrideHitVelocity, Vector3 overrideClubDirection)
        {
            ...
            // Capture the horizontal (pre-loft) travel direction. Face angle, momentum loss and side
            // spin are all measured in the horizontal plane - they must NOT include the vertical tilt
            // that loft adds below, otherwise high-loft clubs (wedges) register as "off-square" even on
            ...
            if (isAttachedToClub)
            {
                ...
                if (faceDirection.sqrMagnitude > 0f)
                {
                    ...
                    if (hasGravity && golfClub.ClubType != GolfClubType.Putter)
                    {
                        ...
                        // Backspin from loft - higher loft and faster swings = more backspin, which the
                        // ball turns into (capped) lift so lofted shots float, carry and land soft. This
                        // is generated regardless of face angle (even a dead-square strike has backspin).
